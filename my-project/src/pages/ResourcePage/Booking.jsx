...
const Booking = ({ onClose, resource }) => {
  ...
  return (
    <div className=" text-green-500 z-40 fixed w-full h-full flex justify-center md:items-center backdrop-blur-sm">
      <div className="w-3/4 h-auto overflow-hidden bg-green-200 rounded-2xl p-6 shadow-xl flex flex-col">
        ...
        <div className="w-full h-full flex flex-row gap-6">
          <motion.div
            ...
            className={` ${timesOpen ? "hidden" : ""} w-full h-auto bg-white rounded-lg shadow-lg flex flex-col gap-4 items-center justify-center p-4`}
          >
            <img
              src={resource.img}
              alt={resource.name}
          ...
          <motion.div
            ...
            transition={{ duration: 0.5, delay: 0, type: "spring", stiffness: 100, ease: "easeIn" }}
            className={` ${timesOpen ? "" : "hidden"} w-full h-auto bg-white rounded-lg shadow-lg flex flex-col gap-4 p-4`}
          >
            <h1 className="text-xl font-bold text-green-500 ">{resource.name}</h1>
            <h1 className="text-sm text-gray-400">Slots: {selectedTimes.length}</h1>
            <h1 className="text-sm text-gray-400">Contributors: {contributors.length}</h1>
            <form className="w-full flex justify-center items-center mt-4">
              <input
                type="text"
                className="w-full  bg-green-200 text-green-500 p-4 rounded-md"
                placeholder="Search users"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </form>
            <div className="w-full h-32 flex flex-col items-center gap-y-4 rounded-md bg-green-200 p-4 overflow-y-scroll mt-4">
              {searchTerm.length ? filteredUsers.map((user, index) => (
                <motion.button
                  ...
                  key={index}
                  className="bg-white rounded-lg text-green-500 font-bold justify-between flex items-center p-2 w-full"
                >
          ...
          <motion.div
            ...
            className={` ${timesOpen ? "hidden" : ""} w-full bg-white rounded-lg flex flex-col items-center justify-center p-4`}
          >
            <h1 className="text-2xl  font-bold text-center text-green-500 mb-2">{resource.name}</h1>
            ...
            <div className="w-full flex justify-center gap-4">
              <button
                onClick={handleTimes}
                className="px-4 py-2 bg-green-500 text-white rounded-xl hover:bg-green-400 transition duration-300"
              >
                Book Now
              </button>
            </div>
          ...
          >
              ...
              {resource.availableHours.split(",").map((time, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`w-20  text-nowrap h-8 text-xs rounded-lg border border-green-300 bg-green-200 flex  text-center justify-center hover:bg-opacity-50 hover:bg-green-300 hover:text-green-500 items-center ${
                      selectedTimes.includes(time) && !booked ? "bg-gradient-to-br from-green-300 to-green-500 text-green-200" : ""
                    } ${booked ? "text-neutral-500 bg-neutral-200 border-neutral-800 hover:bg-red-300" : ""}`}
            ...
            <div className="w-full h-1/6 items-center justify-between flex gap-4 text-sm text-center py-8">
              <button className="border-green-300 border p-2 rounded-lg" onClick={() => setTimesOpen(false)}>
                <IoArrowBackSharp />
              </button>
              <button className="border-green-300 border p-2 rounded-lg" onClick={sendData}>Confirm</button>
            </div>
          </motion.div>
        </div>
      </div>
